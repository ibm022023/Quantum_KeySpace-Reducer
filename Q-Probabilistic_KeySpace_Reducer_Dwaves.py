...
║  ── GUPPY — Stabilizer sampling via SELENE / HELIOS ───────────────────── ║
║    guppylang stabilizer simulator (≤ 8 qubits)                              ║
║    HELIOS / SELENE-PyPI via Q-Nexus · SELENE-GitHub offline clone           ║
║    (--backend guppy | option [9] interactive)                                ║
║  ── D-WAVE — QUBO Ising minimisation via Leap / SimulatedAnnealing ──────  ║
║    Real D-Wave QPU via Leap cloud (DWAVE_API_TOKEN)                          ║
║    Neal simulated annealing (local, no token needed)                         ║
║    (--backend dwave / dwave_real | options [10]/[11] interactive)            ║
║                                                                              ║
...
║    pip install guppylang                               # GUPPY/SELENE       ║
║    pip install dwave-ocean-sdk neal dimod              # D-Wave / Neal      ║
║  Tokens:  export IQM_TOKEN=...   export IBM_QUANTUM_TOKEN=...               ║
║           export DWAVE_API_TOKEN=...   (D-Wave Leap; optional)              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ── Guppy / SELENE ────────────────────────────────────────────────────────────
import subprocess as _subprocess_guppy
try:
    from guppylang import guppy
    from guppylang.std.builtins import comptime, array, result
    from guppylang.std.quantum import h, x, cx, ry, measure, reset, discard, qubit
    GUPPY_OK = True
except ImportError:
    GUPPY_OK = False

# ── D-Wave (full Leap SDK) ────────────────────────────────────────────────────
try:
    from dwave.cloud import Client as DWaveClient
    from dwave.system import DWaveSampler, EmbeddingComposite
    import dimod
    DWAVE_OK = True
except ImportError:
    DWAVE_OK = False
    DWaveClient = None   # type: ignore
    DWaveSampler = None  # type: ignore
    EmbeddingComposite = None  # type: ignore

# ── D-Wave simulated annealing (neal — local, no login needed) ────────────────
try:
    from neal import SimulatedAnnealingSampler as NealSampler
    NEAL_OK = True
except ImportError:
    try:
        from dwave.samplers import SimulatedAnnealingSampler as NealSampler
        NEAL_OK = True
    except ImportError:
        NEAL_OK = False
        NealSampler = None  # type: ignore

# ── dimod (BQM even without full Leap SDK) ────────────────────────────────────
try:
    import dimod as _dimod_mod  # noqa: F811
    DIMOD_OK = True
except ImportError:
    DIMOD_OK = False
    _dimod_mod = None  # type: ignore
...
log.info("=" * 72)
log.info(f"  GUPPY_OK={GUPPY_OK}  DWAVE_OK={DWAVE_OK}  NEAL_OK={NEAL_OK}  DIMOD_OK={DIMOD_OK}")
...
# =============================================================================
# LATTICE REFINEMENT  [G-8: BKZ block 20+ · Babai CVP · Noise Filter · 23-CF]
# Upgraded with algorithms from P11-REGEV-ULTIMATE v3:
#   • noise_tolerant_filter  — Gaussian reweighting before BKZ (Ragavan-VV)
#   • continued_fraction_approx — 23-depth CF convergents (vs 1 before)
#   • babai_nearest_plane    — real Babai CVP solver on BKZ-reduced lattice
#   • Stratified BKZ sample  — top-N + evenly-spaced tail (no hard truncation)
# =============================================================================
...
def noise_tolerant_filter(
    counts: Dict[str, int],
    bits: int,
    sigma: float = 2.0,
) -> Dict[str, int]:
    """
    Gaussian reweighting of QPU shot histogram before lattice reduction.

    Ported from P11-REGEV-ULTIMATE v3 (Ragavan-Vaikuntanathan 2024).
    Outcomes near 0 or 2^bits (the expected QPE peaks for a good measurement)
    are upweighted by exp(-dist^2 / (2σ^2)).  Noise outcomes far from zero
    are downweighted but NEVER dropped (minimum weight = 30%).

    Applied in Phase D before BKZ so the lattice sees the strongest signal
    candidates at the top — making BKZ converge faster on the right vector.

    sigma=0 disables filtering (returns counts unchanged).
    sigma=2.0 is the default recommended by Ragavan & Vaikuntanathan.
    """
    if sigma <= 0:
        return counts
    import math as _m
    ...
    for bs, cnt in counts.items():
        ...
        weight = _m.exp(-0.5 * (dist / max(sigma_abs, 1.0)) ** 2) if sigma_abs > 0 else 1.0
        new_cnt = max(1, round(cnt * (0.30 + 0.70 * weight)))
        filtered[bs] = filtered.get(bs, 0) + new_cnt
    log.info(f"  [NoiseFilter] σ={sigma:.1f}  σ_abs={sigma_abs:.0f}  "
             f"{len(counts)} → {len(filtered)} outcomes (reweighted)")
    return filtered


def continued_fraction_approx(num: int, den: int, max_den: int = 1_000_000):
    """
    Multi-depth continued-fraction rational approximation.

    Ported from P11-REGEV-ULTIMATE v3.
    Returns the `den`-th convergent of num/max_den via the standard
    Euclidean CF algorithm.  `den` here acts as the depth index (1..23).
    Used to extract all rational fractions p/q with small q that are
    close to the measured QPE phase, catching cases where single-depth
    Fraction.limit_denominator() misses the correct k/n rational.
    """
    from fractions import Fraction
    if max_den == 0:
        return 0, 1
    frac = Fraction(num, max_den).limit_denominator(den * max_den // max(den, 1))
    # Standard: compute the den-th convergent explicitly
    a0, rem = num, max_den
    p_prev, p_curr = 1, 0
    q_prev, q_curr = 0, 1
    for _ in range(den):
...
def babai_nearest_plane(
    ...
    """
    Babai nearest-plane CVP solver on a BKZ/LLL-reduced lattice basis.

    Ported from P11-REGEV-ULTIMATE v3.
    After BKZ gives a short, well-reduced basis, Babai's algorithm finds
    the lattice vector closest to `target_vec` by Gram-Schmidt projection
    — a much tighter approximation than reading off BKZ rows directly.

    Returns a list of candidate integers modulo `order`.
    Falls back silently if fpylll GSO is unavailable.
    """
    if not FPYLLL_OK:
        return []
    try:
        ...
        for i in range(n_rows - 1, -1, -1):
            ...
            if bstar_norm_sq <= 0:
                continue
            dot = sum(b[j] * float(M[i, j]) for j in range(n_cols))
            mu  = dot / bstar_norm_sq
            c   = round(mu)
...
def lattice_refine_candidates(
    ...
    """
    Upgraded lattice refinement [G-8].

    Improvements over the original:
      1. Noise-tolerant Gaussian filter on the raw offset histogram
         before BKZ (Ragavan-VV) — sharpens the signal for BKZ to follow.
      2. Stratified BKZ sample: top-N by energy + evenly-spaced tail picks
         instead of a hard truncation — catches medium-energy offsets that
         carry the real signal on noisy hardware.
      3. Real Babai nearest-plane CVP on top-20 target vectors after BKZ
         — finds the closest lattice point to each target, which is
         mathematically tighter than reading off BKZ rows directly.
      4. Extracts top-10 rows after each BKZ pass (was 8).
    """
    if len(top_offsets) < 2:
        return []
    extra: List[Tuple[float, int]] = []
    order = 1 << bits

    # ── Apply noise-tolerant filter to offset energy map ────────────────────
    # Convert offset list to a pseudo-histogram for the filter
    _offset_counts: Dict[str, int] = {
        bin(off)[2:].zfill(bits): 1 for off in top_offsets
    }
    _filtered = noise_tolerant_filter(_offset_counts, bits, sigma=noise_sigma)
    # Rebuild weighted offset list from filtered counts
    _filtered_offsets = []
    for bs, w in _filtered.items():
        try:
            off = int(bs, 2)
            for _ in range(max(1, w)):
                _filtered_offsets.append(off)
        except ValueError:
            pass
    # Deduplicate but preserve weight ordering
    _seen: set = set()
    weighted_offsets: List[int] = []
    for off in _filtered_offsets:
        if off not in _seen:
            _seen.add(off)
            weighted_offsets.append(off)
    if not weighted_offsets:
        weighted_offsets = list(top_offsets)

    n = len(weighted_offsets)

    if FPYLLL_OK and n >= 2:
        try:
            # Stratified sample: top half + evenly-spaced tail
            N_BKZ  = max(200, min(1500, n))
            n_top  = min(N_BKZ // 2, n)
            n_tail = min(N_BKZ - n_top, n - n_top)
            sampled = weighted_offsets[:n_top]
            if n_tail > 0 and len(weighted_offsets) > n_top:
                tail   = weighted_offsets[n_top:]
                step   = max(1, len(tail) // n_tail)
                sampled += tail[::step][:n_tail]

            d     = max(2, min(bits // 4, len(sampled)))
            chunk = max(1, bits // max(1, d))
            M     = IntegerMatrix(len(sampled), d)
            for i, off in enumerate(sampled):
                for j in range(d):
            ...
            # Extract candidates from top-10 rows (was 8)
            for row in range(min(10, M.nrows)):
                cand = abs(sum(M[row, j] * (1 << (j * chunk))
                               for j in range(d))) % order
                e = compute_energy(base + cand, target_h160, pubkey_xy, pubkey_hex)
                extra.append((e, cand))

            # Babai nearest-plane CVP on top-20 target vectors
            n_babai = min(20, len(sampled))
            babai_hits = 0
            for trial in range(n_babai):
                target = [sampled[trial] >> (j * chunk) & ((1 << chunk) - 1)
                          for j in range(d)]
                br = babai_nearest_plane(M, target, order)
                if br:
                    for bv in br:
                        e = compute_energy(base + bv, target_h160,
                                           pubkey_xy, pubkey_hex)
                        extra.append((e, bv))
                    babai_hits += 1
            log.info(f"  [Lattice-BKZ{bkz_block}] {len(extra)} candidates  "
                     f"(stratified={len(sampled)}/{n}  babai={babai_hits} targets  "
                     f"σ={noise_sigma:.1f} filter)")
        except Exception as ex:
            log.debug(f"  [Lattice-BKZ] error: {ex}")
    else:
        # Scalar 2×2 LLL fallback — apply to all weighted offsets
        for off in weighted_offsets[:min(1500, n)]:
...
def _qft_frequency_to_key(freq: int, bits: int, base: int,
    ...
    UPGRADED: 23-depth continued-fraction convergents (Shor CF recovery).
    Ported from P11-REGEV-ULTIMATE v3.

    On noisy hardware the measured QPE phase m/2^bits may not be the single
    best rational approximation at depth 1 — deeper convergents (depth 2..23)
    catch the correct k/n fraction that single-depth Fraction.limit_denominator
    misses.  For each depth dd=1..23 we compute the dd-th CF convergent p/q
    of freq/2^bits, then map: k = full_start + round(p * space / q).
    The first in-range candidate is returned.
    """
    ...
    # Depth 0: direct linear mapping
    k_direct = full_start + round(freq * space / N)
    candidates.append(k_direct)

    # Depths 1..23: CF convergents of freq / N
    if freq > 0:
        a0, rem     = freq, N
        p_prev, p_curr = 1, 0
        q_prev, q_curr = 0, 1
        for _depth in range(23):
            if rem == 0:
                break
            a       = a0 // rem
            a0, rem = rem, a0 - a * rem
            ...
            q_prev, q_curr = q_curr, a * q_curr + q_prev
            if q_curr > 0:
                k_cf = full_start + round(p_curr * space / q_curr)
                candidates.append(k_cf)
                # direct modular mapping of this convergent numerator
                k_mod = (full_start + p_curr) % max(full_end + 1, 1)
                if full_start <= k_mod <= full_end:
                    candidates.append(k_mod)
...
def _strip_mid_circuit_for_iqm(qc: "QuantumCircuit") -> "QuantumCircuit":
    """
    Remove mid-circuit measures, resets, and if_test blocks so the circuit
    satisfies IQM's CommutableMeasuresPredicate.

    Ported from P11-REGEV-ULTIMATE v3 (strip_mid_circuit_for_iqm).

    IQM hardware via pytket does not support dynamic/adaptive circuits in the
    standard submission path.  Any mid-circuit measure, reset, or if_test in
    the circuit causes CommutableMeasuresPredicate to fail before submission.

    Transformation rules:
      reset(q)              → dropped (qubit is |0⟩ at start)
      measure(q,c) [mid]    → dropped (re-added as terminal at end)
      if_test block         → dropped (open-loop: no feed-forward on IQM)
      measure(q,c) [final]  → kept
    A single consolidated ClassicalRegister ("c_iqm") is created with one
    bit per qubit that was measured anywhere in the original circuit.
    """
    try:
        ...
        nq = qc.num_qubits
        # Collect every qubit that was ever measured
        ...
        for instr in qc.data:
            op    = instr.operation
            qargs = instr.qubits
            if isinstance(op, (Reset, IfElseOp)):
                continue
            if isinstance(op, Measure):
                continue   # re-added below as terminal layer
        ...
        # Single terminal measurement layer
        for bit_idx, q_idx in enumerate(sorted(measured_qubits)):
            new_qc.measure(new_qc.qubits[q_idx], c_iqm[bit_idx])

        log.info(f"  [IQM-strip] {qc.num_qubits}q depth={qc.depth()} → "
                 f"{new_qc.num_qubits}q depth={new_qc.depth()} "
                 f"(mid-circuit measures/resets/if_test removed)")
        return new_qc
    except Exception as ex:
        log.warning(f"  [IQM-strip] failed ({ex}) — returning original circuit")
        return qc


def _decompose_for_pytket(qc: "QuantumCircuit") -> "QuantumCircuit":
    """
    Expand QFTGate and other Qiskit compound gates into primitives that
    pytket's qiskit_to_tk converter understands.

    Ported from P11-REGEV-ULTIMATE v3 (decompose_for_pytket).

    pytket.extensions.qiskit.qiskit_to_tk has a hard-coded lookup table
    (_known_qiskit_gate).  QFTGate (Qiskit >= 1.1) and MCXGate are
    NOT in that table, raising:
    ...
    MCXGate → CCX → CX+T, etc.).  The final transpile call maps every gate
    to a set pytket recognises for certain.
    """
    try:
        ...
        qc_prim = _transpile(qc_dec, basis_gates=PYTKET_BASIS, optimization_level=0)
        log.info(f"  [pytket-decompose] depth {qc.depth()} → {qc_prim.depth()} "
                 f"(QFTGate and compounds expanded)")
        return qc_prim
    except Exception as ex:
        log.warning(f"  [pytket-decompose] failed ({ex}) — returning original circuit")
        return qc
...
def _run_on_iqm(
    ...
    UPGRADED with P11-REGEV-ULTIMATE v3 hardening:
      Step 1: strip_mid_circuit_for_iqm — removes mid-circuit measures,
              resets, if_test (CommutableMeasuresPredicate fix).
      Step 2: decompose_for_pytket — expands QFTGate and MCXGate into
              primitives that qiskit_to_tk recognises (QFT conv. fix).
      Step 3: qiskit_to_tk() — Qiskit → pytket.
      Step 4: get_compiled_circuit() with opt level retry (0→1→2).
              Validates with backend.valid_circuit() before submitting
              (ConnectivityPredicate fix).
      Step 5: process_circuit() → get_result() → counts.
    """
    if not PYTKET_QISKIT_OK:
        raise RuntimeError("pytket-qiskit not installed: pip install pytket-qiskit")

    # Step 1: remove mid-circuit operations IQM cannot handle
    qiskit_circuit = _strip_mid_circuit_for_iqm(qiskit_circuit)

    # Step 2: expand compound gates pytket does not know
    qiskit_circuit = _decompose_for_pytket(qiskit_circuit)

    # Step 2b: Clifford+T 2q-block consolidation (KAK) before pytket conversion
    # Reduces CNOT count before routing → fewer SWAPs → better fidelity on IQM.
    qiskit_circuit = cliffordT_optimize(qiskit_circuit)

    # Step 3: convert to pytket
    tk_circ = qiskit_to_tk(qiskit_circuit)

    # Step 4: compile with validation retry
    compiled = None
    for opt_level in (2, 1, 0):
        try:
            candidate = iqm_backend.get_compiled_circuit(
                tk_circ, optimisation_level=opt_level)
            try:
                valid = iqm_backend.valid_circuit(candidate)
            except Exception:
                valid = True   # valid_circuit not available in all versions
            if valid:
                log.info(f"  [IQM] compiled OK (opt_level={opt_level})")
                compiled = candidate
                break
            else:
                log.warning(f"  [IQM] opt_level={opt_level} → ConnectivityPredicate fail, retry")
        except Exception as ex:
            log.warning(f"  [IQM] compile opt_level={opt_level} error: {ex}")

    if compiled is None:
        raise RuntimeError(
            "IQM routing failed at all optimisation levels (0,1,2). "
            "Circuit may be too large for the selected device. "
            "Try a smaller bit-length or adder=approx."
        )

    # Step 5: submit and retrieve
...
# =============================================================================
# CLIFFORD+T CIRCUIT OPTIMIZATION  [Ported from P11-REGEV-ULTIMATE v3]
# Applies 2q-block consolidation (KAK decomposition) + cancellation passes
# before IQM/IBM submission — same optimisation pytket does internally.
# Reduces 2-qubit gate count and circuit depth without changing semantics.
# Call cliffordT_optimize(qc) on any circuit before _run_on_iqm / _run_on_ibm.
# =============================================================================

def cliffordT_optimize(qc: "QuantumCircuit") -> "QuantumCircuit":
    """
    Clifford+T optimization with 2q-block consolidation (KAK decomposition).

    Ported from P11-REGEV-ULTIMATE v3 (Google/Schrottenloher edition).

    Pass order:
      1. Decompose CCX/MCX → CX+T primitives
      ...
      3. ConsolidateBlocks — re-synthesize each block as minimal KAK (≤3 CNOT)
         (this is what pytket does internally with FullPeepholeOptimise)
      4. CommutativeCancellation — gate-pair removal
      5. InverseCancellation — CX+CX = I, H+H = I, T+Tdg = I etc.
      6. Optimize1qGates — merge/cancel 1-qubit rotation chains
      7. Second CommutativeCancellation — catches new pairs after 1q merge

    For Aer simulator: cosmetic (simulation is exact regardless).
    For IQM hardware: reduces 2-qubit gate count → fewer SWAP insertions
      during pytket routing → lower final depth → better fidelity.
    For IBM hardware: reduces depth before routing → avoids error 6057
      (IBM's JIT compiler crash above ~15k gates after routing).

    Falls back silently to the original circuit if any pass fails.
    """
    try:
        from qiskit.transpiler import PassManager
        ...
        BASIS = [
            "rz", "sx", "x", "cx", "t", "tdg", "h", "s", "sdg",
            "cp", "p", "cz", "u", "u1", "u2", "u3", "ry", "rx",
            "reset", "measure", "if_else",
        ]

        # Try to import 2q-block consolidation (Qiskit >= 0.45)
        try:
            ...
            consolidation_passes = [
                UnrollCustomDefinitions(_sel, BASIS),
                Collect2qBlocks(),
                ConsolidateBlocks(basis_gates=BASIS),
            ]
            log.info("  [CliffordT] 2q-block consolidation available (KAK decomposition)")
        except ImportError:
            consolidation_passes = []
            log.info("  [CliffordT] Collect2qBlocks unavailable — skipping consolidation")
        ...
        pm = PassManager(
            ...
            + [
                ...
                CommutativeInverseCancellation(),
                InverseCancellation(
                    gates_to_cancel=[CXGate(), HGate(), (TGate(), TdgGate())]
                ),
        ...
        out  = pm.run(qc)
        ops  = out.count_ops()
        t_c  = ops.get("t", 0) + ops.get("tdg", 0)
        cx_c = ops.get("cx", 0) + ops.get("cp", 0) + ops.get("cz", 0)
        log.info(
            f"  [CliffordT] T={t_c}  2q={cx_c}  total={sum(ops.values())}  "
            f"depth={out.depth()}  "
            f"(was {qc.count_ops().get('cx',0)+qc.count_ops().get('cp',0)} 2q  "
            f"depth={qc.depth()})"
        )
        return out
    except Exception as ex:
        log.warning(f"  [CliffordT] optimization failed ({ex}) — original circuit kept")
        return qc
...
# =============================================================================
# ══════════════════════════════════════════════════════════════════════════════
# BACKEND — GUPPY / SELENE  (Stabilizer sampling, ≤ 8 qubits)
# ══════════════════════════════════════════════════════════════════════════════
# =============================================================================

def _run_guppy_kernel(bits: int, shots: int) -> "Counter":
    """
    Run a Guppy stabilizer kernel that generates a uniform superposition over
    `bits` qubits and returns bitstring counts.
    Limited to ≤ 8 qubits for the stabilizer simulator.
    Falls back to classical random sampling if guppylang is not installed.
    """
    ...
    @guppy
    def prob_kernel() -> None:
        qs = array(qubit() for _ in range(_N))
        for i in comptime(range(_N)):
            h(qs[i])
        for i in comptime(range(_N - 1)):
            cx(qs[i], qs[i + 1])
        for i in comptime(range(_N)):
            m = measure(qs[i])
            result(comptime(f"q{i}"), m)
            discard(qs[i])

    em = (
        prob_kernel
        .emulator(n_qubits=_N)
        .stabilizer_sim()
        .with_shots(shots)
        .run()
    )
    raw: "Counter" = _Counter()
    try:
        for shot in em:
            bs = "".join(
                "1" if shot.get(f"q{i}", False) in (True, 1, "1") else "0"
                for i in range(_N)
            )
            raw[bs] += 1
    except Exception:
        try:
            for tag_tuple, cnt in em.collated_counts().items():
                d  = dict(tag_tuple)
                bs = "".join(
                    "1" if d.get(f"q{i}", False) in (True, 1, "1") else "0"
                    for i in range(_N)
                )
                raw[bs] += cnt
        except Exception as e:
            log.warning(f"  [Guppy] result parsing failed: {e}")
    return raw


def run_guppy_backend(
    bits:          int,
    base:          int,
    target_h160:   bytes,
    pubkey_xy:     Optional[Tuple[int, int]] = None,
    ...
    iterations:    int = 12,
    shots:         int = 4096,
    energy_thresh: float = 0.30,
    pin_high:      float = 0.85,
    pin_low:       float = 0.15,
    guppy_backend: str = "selene_github",
) -> List[Tuple[float, int]]:
    """
    Guppy / SELENE probabilistic backend.

    Stabilizer simulator limited to ≤ 8 bits native.
    For larger bit sizes, falls back gracefully to classical random sampling.

    Returns a list of (energy, offset) pairs compatible with the main
    `sampler_samples` pool consumed by Phase D (quantum_trajectory_range).

    Backends:
      selene_github  — clones Quantinuum/selene.git offline (default)
      selene_pypi    — login via Q-Nexus PyPI package
      helios         — Quantinuum H-Series cloud via Q-Nexus
    """
    log.info("=" * 60)
    log.info(f"  [GUPPY BACKEND] bits={bits}  backend={guppy_backend}  "
             f"iters={iterations}  shots={shots}")

    use_guppy = GUPPY_OK and bits <= 8
    if bits > 8:
        log.warning(
            f"  [GUPPY] Stabilizer simulator limited to ≤ 8 qubits.  "
            f"bits={bits} > 8 — classical random fallback."
        )

    # ── HELIOS / SELENE-PyPI login ────────────────────────────────────────────
    if use_guppy and guppy_backend in ("helios", "selene_pypi"):
        try:
            import qnexus as qnx
            if not (hasattr(qnx, "is_authenticated") and qnx.is_authenticated()):
                qnx.login()
            log.info(f"  [Guppy] Logged into Q-Nexus ({guppy_backend})")
        except Exception as e:
            log.warning(f"  [Guppy] Q-Nexus login failed: {e} — using SELENE local")
            guppy_backend = "selene_github"

    # ── SELENE-GitHub clone ───────────────────────────────────────────────────
    if use_guppy and guppy_backend == "selene_github":
        local_path = "selene"
        if not os.path.exists(local_path):
            try:
                log.info("  [Guppy] Cloning Selene GitHub repo...")
                _subprocess_guppy.run(
                    ["git", "clone",
                     "https://github.com/Quantinuum/selene.git", local_path],
                    check=True, capture_output=True,
                )
                sys.path.insert(0, os.path.abspath(
                    os.path.join(local_path, "selene-sim")
                ))
            except Exception as e:
                log.warning(f"  [Guppy] Selene clone failed: {e}")

    pairs: List[Tuple[float, int]] = []
    best_energy = float("inf")

    for it in range(iterations):
        log.info(f"  [Guppy] Iteration {it + 1}/{iterations}")

        if use_guppy:
            try:
                from collections import Counter as _C
                counts: "Counter" = _run_guppy_kernel(bits, shots)
            except Exception as e:
                log.warning(f"  [Guppy] kernel error: {e} — random fallback")
                from collections import Counter as _C
                counts = _C({
                    format(int(x), f"0{bits}b"): 1
                    for x in np.random.randint(0, 1 << bits, shots)
                })
        else:
            from collections import Counter as _C
            counts = _C({
        ...
        for bitstr, cnt in counts.items():
            ...
            offset = int(bs, 2)
            k      = base + offset
            energy = compute_energy(k, target_h160, pubkey_xy, pubkey_hex)
            if energy < best_energy:
                best_energy = energy
            if energy <= energy_thresh:
                for _ in range(min(cnt, 10)):
                    pairs.append((energy, offset))
                n_low += 1

        log.info(f"    best_e={best_energy:.6f}  low-E pairs this iter: {n_low}")

    log.info(f"  [GUPPY BACKEND] {len(pairs)} low-energy pairs collected  "
             f"(best_e={best_energy:.6f})")
    return pairs


# =============================================================================
# ══════════════════════════════════════════════════════════════════════════════
# BACKEND — D-WAVE  (QUBO Ising minimisation via Leap / SimulatedAnnealing)
# ══════════════════════════════════════════════════════════════════════════════
# =============================================================================

def _build_ising_bqm_dwave(
    bits:        int,
    base:        int,
    target_h160: bytes,
    pubkey_xy:   Optional[Tuple[int, int]] = None,
    ...
    """
    Build a Binary Quadratic Model (BQM/QUBO) for D-Wave / Neal.

    Strategy — same energy landscape as the Ising Hamiltonian in Phase A:
      1. Sample n_probes random keys in the bit range.
      2. For each, compute energy(k, target).
      3. Fit per-bit linear biases and nearest-neighbour quadratic couplings.

    In pubkey mode the BQM has strong structure (algebraic EC distance).
    In hash160-only mode it is noisy (hash destroys algebraic structure).
    """
    import dimod as _dimod

    log.info(f"  [D-Wave BQM] Building {bits}-variable BQM  "
             f"({n_probes} probes)…")
    offsets  = _sobol_offsets(n_probes, bits)
    energies = np.array([
        compute_energy(base + o, target_h160, pubkey_xy, pubkey_hex)
        for o in offsets
    ], dtype=float)
    e_min, e_max = energies.min(), energies.max()
    en = (
        (energies - e_min) / (e_max - e_min)
        if e_max > e_min else np.full(n_probes, 0.5)
    )

    bqm = _dimod.BinaryQuadraticModel(vartype="BINARY")

    # Linear biases — per-bit marginal energy
    bit_energy = np.zeros(bits)
    bit_count  = np.zeros(bits)
    ...
    for b in range(bits):
        if bit_count[b] > 0:
            avg = bit_energy[b] / bit_count[b]
            # Positive bias → discourage bit=1 when it correlates with high energy
            bqm.add_variable(b, float(avg - 0.5))

    # Quadratic couplings — nearest-neighbour
    for b in range(bits - 1):
        same = np.array([
            ((o >> b) & 1) == ((o >> (b + 1)) & 1)
            for o in offsets
        ...
        e_s = en[same]; e_d = en[~same]
        J = ((e_d.mean() if len(e_d) else 0.5) -
             (e_s.mean() if len(e_s) else 0.5))
        bqm.add_interaction(b, b + 1, float(J))

    log.info(f"  [D-Wave BQM] Built: {len(bqm.variables)} variables  "
             f"{len(bqm.quadratic)} interactions")
    return bqm


def run_dwave_backend(
    bits:            int,
    base:            int,
    target_h160:     bytes,
    pubkey_xy:       Optional[Tuple[int, int]] = None,
    ...
    energy_thresh:   float = 0.30,
    dwave_token:     str = "",
    dwave_endpoint:  str = "https://cloud.dwavesys.com/sapi/",
    use_real_dwave:  bool = False,
) -> List[Tuple[float, int]]:
    """
    D-Wave backend — QUBO Ising minimisation.

    Sampler priority:
      1. Real D-Wave QPU via Leap cloud (requires dwave_token + use_real_dwave=True).
      2. Neal simulated annealing sampler (local, no login).
      3. dimod ExactSolver (tiny BQMs ≤ 20 variables, for CI/testing).

    Returns a list of (energy, offset) pairs compatible with the main
    `sampler_samples` pool consumed by Phase D (quantum_trajectory_range).
    """
    log.info("=" * 60)
    log.info(f"  [D-WAVE BACKEND] bits={bits}  reads={num_reads}  "
             f"use_real={use_real_dwave}")

    if not DIMOD_OK:
        log.error(
            "  [D-Wave] dimod not installed.  "
            "pip install dwave-ocean-sdk  OR  pip install dimod neal"
        )
        return []

    import dimod as _dimod

    bqm = _build_ising_bqm_dwave(
        bits, base, target_h160, pubkey_xy, pubkey_hex, n_probes
    )
    ...
    if use_real_dwave and DWAVE_OK and dwave_token:
        # ── Optional: run D-Wave CLI setup wizard before connecting ──────────
        # dwave config create --full  →  writes ~/.config/dwave/dwave.conf
        # dwave setup --auth          →  verifies token against Leap cloud
        # These are one-time shell steps; skipping them is safe if you already
        # exported DWAVE_API_TOKEN and have no local config file.
        try:
            _run_dwave_setup = input(
                "\n  [D-Wave] Run 'dwave config create --full && dwave setup --auth' "
                "wizard now?\n"
                "  (Writes local config + verifies Leap token — safe to skip if "
                "already done)\n"
                "  Enter Yes or No [No]: "
            ).strip().lower()
        except EOFError:
            _run_dwave_setup = "no"
        if _run_dwave_setup in ("yes", "y"):
            import subprocess as _dwave_setup_sub
            log.info("  [D-Wave] Running: dwave config create --full …")
            _cfg_result = _dwave_setup_sub.run(
                ["dwave", "config", "create", "--full"],
                check=False,
            )
            if _cfg_result.returncode == 0:
                log.info("  [D-Wave] Config written successfully.")
            else:
                log.warning(
                    f"  [D-Wave] 'dwave config create' exited with code "
                    f"{_cfg_result.returncode} — continuing anyway."
                )
            log.info("  [D-Wave] Running: dwave setup --auth …")
            _auth_result = _dwave_setup_sub.run(
                ["dwave", "setup", "--auth"],
                check=False,
            )
            if _auth_result.returncode == 0:
                log.info("  [D-Wave] Auth verified ✅")
            else:
                log.warning(
                    f"  [D-Wave] 'dwave setup --auth' exited with code "
                    f"{_auth_result.returncode} — attempting QPU connect anyway."
                )
        else:
            log.info("  [D-Wave] CLI setup skipped — using token directly.")
        # ── End optional setup wizard ─────────────────────────────────────────
        try:
            log.info("  [D-Wave] Connecting to Leap QPU…")
            _dws = DWaveSampler(
                token=dwave_token,
                endpoint=dwave_endpoint,
            )
            sampler       = EmbeddingComposite(_dws)
            sampler_label = (
                f"D-Wave QPU ({_dws.properties.get('chip_id', '?')})"
            )
            log.info(f"  [D-Wave] Connected: {sampler_label}")
        except Exception as e:
            log.error(f"  [D-Wave] QPU connect failed: {e}")
            log.info("  [D-Wave] → Falling back to Neal")
            use_real_dwave = False

    if not use_real_dwave or sampler is None:
        if NEAL_OK:
            sampler       = NealSampler()
            sampler_label = "Neal SimulatedAnnealingSampler (local)"
            log.info(f"  [D-Wave] Using {sampler_label}")
        elif bits <= 20:
            sampler       = _dimod.ExactSolver()
            sampler_label = "dimod ExactSolver (tiny BQM ≤ 20 vars)"
            num_reads     = 1
            log.info(f"  [D-Wave] Using {sampler_label}")
        else:
            log.error(
                "  [D-Wave] No sampler available and bits > 20 for ExactSolver.  "
                "Install: pip install neal"
            )
            return []
    ...
    try:
        ...
        log.info(f"  [D-Wave] Sampling complete: {len(sampleset)} samples")
    except Exception as e:
        log.error(f"  [D-Wave] Sampling error: {e}")
        return []

    # ── Evaluate samples → (energy, offset) pairs ────────────────────────────
    pairs: List[Tuple[float, int]] = []
    best_energy = float("inf")

    for sample, _bqm_e, occ in sampleset.data(
        ["sample", "energy", "num_occurrences"]
    ):
        offset = sum(int(sample.get(b, 0)) << b for b in range(bits))
        k      = base + offset
        energy = compute_energy(k, target_h160, pubkey_xy, pubkey_hex)
        ...
        if energy <= energy_thresh:
            for _ in range(min(occ, 20)):
                pairs.append((energy, offset))

    log.info(f"  [D-WAVE BACKEND] best_e={best_energy:.6f}  "
             f"{len(pairs)} low-energy pairs collected  ({sampler_label})")
    return pairs
...
def reduce_keyspace(
    ...
    # ── Guppy / SELENE backend ───────────────────────────────────────────────
    use_guppy_backend:   bool = False,  # True → run Guppy and add to pool
    guppy_backend_type:  str  = "selene_github",  # selene_github|selene_pypi|helios
    guppy_shots:         int  = 4096,
    guppy_iters:         int  = 8,
    guppy_energy_thresh: float = 0.30,
    # ── D-Wave / Neal backend ────────────────────────────────────────────────
    use_dwave_backend:   bool = False,  # True → run D-Wave/Neal and add to pool
    dwave_token:         str  = "",
    dwave_endpoint:      str  = "https://cloud.dwavesys.com/sapi/",
    use_real_dwave:      bool = False,  # False = Neal local; True = Leap QPU
    dwave_num_reads:     int  = 1000,
    dwave_n_probes:      int  = 512,
    dwave_energy_thresh: float = 0.30,
    ...
    B2 : [QUANTUM WALK] Discrete-time quantum walk — superior O(√N) exploration
         + optional GUPPY (SELENE stabilizer, ≤8q) backend samples
         + optional D-WAVE (QUBO Ising / Neal annealing) backend samples
    ...
    use_guppy_backend / use_dwave_backend: add extra sample pools from those machines.
    """
    ...
    # ── Engine GUPPY: SELENE / HELIOS stabilizer samples (optional) ──────────
    if use_guppy_backend:
        log.info(f"  [Phase B2 GUPPY] Running Guppy/SELENE backend  "
                 f"iters={guppy_iters}  shots={guppy_shots}  "
                 f"guppy_backend={guppy_backend_type}")
        try:
            guppy_pairs = run_guppy_backend(
                bits=bits, base=base,
                target_h160=target_h160, pubkey_xy=pubkey_xy, pubkey_hex=pubkey_hex,
                iterations=guppy_iters, shots=guppy_shots,
                energy_thresh=guppy_energy_thresh,
                guppy_backend=guppy_backend_type,
            )
            if guppy_pairs:
                sampler_samples = sampler_samples + guppy_pairs
                log.info(f"  [Phase B2 GUPPY] +{len(guppy_pairs):,} Guppy samples → "
                         f"total={len(sampler_samples):,}")
            else:
                log.warning("  [Phase B2 GUPPY] No low-energy pairs returned — "
                            "try increasing guppy_shots or lowering guppy_energy_thresh")
        except Exception as _guppy_ex:
            log.warning(f"  [Phase B2 GUPPY] failed: {_guppy_ex}")
    else:
        log.debug("  [Phase B2 GUPPY] Skipped (use_guppy_backend=False)")

    # ── Engine D-WAVE: QUBO Ising / Neal annealing samples (optional) ────────
    if use_dwave_backend:
        _dwave_tok  = dwave_token or os.getenv("DWAVE_API_TOKEN", "")
        _use_real   = use_real_dwave and bool(_dwave_tok)
        log.info(f"  [Phase B2 D-WAVE] Running D-Wave backend  "
                 f"use_real={_use_real}  reads={dwave_num_reads}  "
                 f"probes={dwave_n_probes}")
        try:
            dwave_pairs = run_dwave_backend(
                bits=bits, base=base,
                target_h160=target_h160, pubkey_xy=pubkey_xy, pubkey_hex=pubkey_hex,
                num_reads=dwave_num_reads, n_probes=dwave_n_probes,
                energy_thresh=dwave_energy_thresh,
                dwave_token=_dwave_tok,
                dwave_endpoint=dwave_endpoint,
                use_real_dwave=_use_real,
            )
            if dwave_pairs:
                sampler_samples = sampler_samples + dwave_pairs
                log.info(f"  [Phase B2 D-WAVE] +{len(dwave_pairs):,} D-Wave samples → "
                         f"total={len(sampler_samples):,}")
            else:
                log.warning("  [Phase B2 D-WAVE] No low-energy pairs returned — "
                            "try increasing dwave_num_reads or lowering dwave_energy_thresh")
        except Exception as _dwave_ex:
            log.warning(f"  [Phase B2 D-WAVE] failed: {_dwave_ex}")
    else:
        log.debug("  [Phase B2 D-WAVE] Skipped (use_dwave_backend=False)")
    ...
    # ── Upgrade 1: noise_tolerant_filter before BKZ ──────────────────────────
    # Apply Gaussian reweighting to the sampler sample histogram so BKZ sees
    # stronger QPE peaks at the top of the matrix (Ragavan-VV 2024).
    _sample_counts: Dict[str, int] = {}
    for _e, _off in sampler_samples:
        _bs = bin(int(_off))[2:].zfill(bits)
        _sample_counts[_bs] = _sample_counts.get(_bs, 0) + 1
    _filtered_counts = noise_tolerant_filter(_sample_counts, bits, sigma=2.0)
    # Rebuild sampler_samples with filtered weights (higher-weight = repeated)
    _filtered_samples: List[Tuple[float, int]] = []
    _off_energy: Dict[int, float] = {int(_o): _e2 for _e2, _o in sampler_samples}
    for _bs, _w in _filtered_counts.items():
        try:
            _off = int(_bs, 2)
            _e2  = _off_energy.get(_off, 1.0)
            for _ in range(max(1, min(_w, 8))):
                _filtered_samples.append((_e2, _off))
        except ValueError:
            pass
    if _filtered_samples:
        log.info(f"  [Phase D NoiseFilter] {len(sampler_samples)} → "
                 f"{len(_filtered_samples)} samples after Gaussian reweighting")
        sampler_samples = _filtered_samples

    top_for_lattice = list(dict.fromkeys(
        int(o) for _, o in sorted(sampler_samples, key=lambda x: x[0])[:grover_top_k]
    ))
    # Pass noise_sigma to lattice — it re-applies the filter internally too
    lattice_samples = lattice_refine_candidates(
        top_for_lattice, bits, base, target_h160, pubkey_xy, pubkey_hex,
        bkz_block=20, noise_sigma=2.0)
...
""")
    ...
    print("  │  ── EXTRA SAMPLING BACKENDS (pool augmentation) ─────────────────  │")
    print(f"  │  [9]  + GUPPY / SELENE   (stabilizer sampler, ≤8q native)        │")
    print(f"  │       {'✅ guppylang ready' if GUPPY_OK else '⚠ pip install guppylang':<60} │")
    print(f"  │  [10] + D-Wave Neal     (local simulated annealing, no token)    │")
    print(f"  │       {'✅ neal ready' if NEAL_OK else '⚠ pip install neal (or dwave-ocean-sdk)':<60} │")
    print(f"  │  [11] + D-Wave QPU      (Leap cloud, requires DWAVE_API_TOKEN)   │")
    print(f"  │       {'✅ dwave-ocean-sdk ready' if DWAVE_OK else '⚠ pip install dwave-ocean-sdk':<60} │")
    print("  └────────────────────────────────────────────────────────────────────┘")
    bk_in = _ask("Backend [1..11] (Guppy/D-Wave ADD to main backend, not replace)", "1")
    ...
    if bk_in in pytket_map:
        ...
        # Guppy augments the currently-default Aer backend
        backend_mode = "simulator"; iqm_device = "garnet"
        print("  ✅ Aer Simulator (primary) + GUPPY / SELENE (extra samples)")
    elif bk_in in ("10", "11"):
        backend_mode = "simulator"; iqm_device = "garnet"
        if bk_in == "10":
            print("  ✅ Aer Simulator (primary) + D-Wave Neal annealing (extra samples)")
        else:
            print("  ✅ Aer Simulator (primary) + D-Wave QPU via Leap (extra samples)")
    else:
        backend_mode = "simulator"; iqm_device = "garnet"
    ...
    # ── Guppy configuration ───────────────────────────────────────────────────
    _use_guppy        = (bk_in == "9")
    _guppy_bk_type    = "selene_github"
    _guppy_shots      = 4096
    _guppy_iters      = 8
    _guppy_eth        = 0.30
    if _use_guppy:
        print("\n  ┌─ GUPPY / SELENE PARAMETERS ──────────────────────────────────────┐")
        print("  │  Sub-backend: [1] SELENE-GitHub · [2] SELENE-PyPI · [3] HELIOS  │")
        print("  └───────────────────────────────────────────────────────────────────┘")
        _gb = _ask("Guppy sub-backend [1/2/3]", "1")
        _guppy_bk_type = (
            "selene_github" if _gb == "1"
            else "selene_pypi" if _gb == "2"
            else "helios"
        )
        _guppy_shots = int(_ask("Guppy shots per iter", "4096"))
        _guppy_iters = int(_ask("Guppy iterations", "8"))
        _guppy_eth   = float(_ask("Guppy energy threshold (default 0.30)", "0.30"))

    # ── D-Wave / Neal configuration ───────────────────────────────────────────
    _use_dwave       = (bk_in in ("10", "11"))
    _dwave_tok       = ""
    _dwave_endpoint  = "https://cloud.dwavesys.com/sapi/"
    _use_real_dwave  = (bk_in == "11")
    _dwave_reads     = 1000
    _dwave_probes    = 512
    _dwave_eth       = 0.30
    if _use_dwave:
        print("\n  ┌─ D-WAVE / NEAL PARAMETERS ────────────────────────────────────────┐")
        if _use_real_dwave:
            _dwave_tok = (
                os.getenv("DWAVE_API_TOKEN", "")
                or _ask("D-Wave Leap API token", "")
            )
            _ep = _ask(f"D-Wave endpoint [Enter={_dwave_endpoint}]", "")
            if _ep.strip():
                _dwave_endpoint = _ep.strip()
            # ── Optional: D-Wave CLI setup wizard ─────────────────────────────
            # Runs 'dwave config create --full' (writes ~/.config/dwave/dwave.conf)
            # followed by 'dwave setup --auth' (verifies your Leap token online).
            # Safe to skip if you already ran this once or set DWAVE_API_TOKEN.
            print("  │")
            print("  │  [D-Wave CLI Setup — Optional]")
            print("  │  'dwave config create --full' writes your local config file.")
            print("  │  'dwave setup --auth'         verifies your Leap token online.")
            print("  │  Skip if you already ran these or DWAVE_API_TOKEN is exported.")
            _setup_ans = _ask("  Run D-Wave CLI wizard now? (Yes / No)", "No").strip().lower()
            if _setup_ans in ("yes", "y"):
                import subprocess as _dwave_isetup
                print("  │  ── Running: dwave config create --full …")
                _r1 = _dwave_isetup.run(["dwave", "config", "create", "--full"], check=False)
                if _r1.returncode == 0:
                    print("  │  ✅ Config created successfully.")
                else:
                    print(f"  │  ⚠  Exit code {_r1.returncode} — continuing anyway.")
                print("  │  ── Running: dwave setup --auth …")
                _r2 = _dwave_isetup.run(["dwave", "setup", "--auth"], check=False)
                if _r2.returncode == 0:
                    print("  │  ✅ Auth verified.")
                else:
                    print(f"  │  ⚠  Exit code {_r2.returncode} — will attempt QPU connect anyway.")
            else:
                print("  │  ↷  CLI wizard skipped — using token directly.")
            # ── End optional setup wizard ──────────────────────────────────────
        _dwave_reads  = int(_ask("D-Wave num_reads / Neal sweeps", "1000"))
        _dwave_probes = int(_ask("BQM probes (Sobol landscape samples)", "512"))
        _dwave_eth    = float(_ask("Energy threshold (default 0.30)", "0.30"))
        print("  └───────────────────────────────────────────────────────────────────┘")
    ...
    if _use_guppy:
        bkend_str += f" + Guppy/{_guppy_bk_type}"
    if _use_dwave:
        bkend_str += f" + D-Wave/{'QPU' if _use_real_dwave else 'Neal'}"
    print(f"\n{'─'*74}")
    ...
    print(f"  GUPPY_OK     : {'✅ guppylang ready' if GUPPY_OK else '— pip install guppylang'}  "
          f"| Active: {'✅' if _use_guppy else '—'}")
    print(f"  DWAVE_OK     : {'✅ dwave-ocean-sdk ready' if DWAVE_OK else '— pip install dwave-ocean-sdk'}  "
          f"| NEAL_OK: {'✅' if NEAL_OK else '— pip install neal'}  "
          f"| Active: {'✅ QPU' if (_use_dwave and _use_real_dwave) else '✅ Neal' if _use_dwave else '—'}")
    ...
    kw  = dict(
        ...
        # ── Guppy / SELENE ───────────────────────────────────────────────────
        use_guppy_backend   = _use_guppy,
        guppy_backend_type  = _guppy_bk_type,
        guppy_shots         = _guppy_shots,
        guppy_iters         = _guppy_iters,
        guppy_energy_thresh = _guppy_eth,
        # ── D-Wave / Neal ────────────────────────────────────────────────────
        use_dwave_backend   = _use_dwave,
        dwave_token         = _dwave_tok,
        dwave_endpoint      = _dwave_endpoint,
        use_real_dwave      = _use_real_dwave,
        dwave_num_reads     = _dwave_reads,
        dwave_n_probes      = _dwave_probes,
        dwave_energy_thresh = _dwave_eth,
    )
...
""".format(s=IQM_DEVICE_QUBITS['sirius'], g=IQM_DEVICE_QUBITS['garnet'], e=IQM_DEVICE_QUBITS['emerald']))
    ...
    # ── Guppy backend ─────────────────────────────────────────────────────────
    p.add_argument("--guppy",           action="store_true",
                   help="Add Guppy/SELENE stabilizer samples to pool")
    p.add_argument("--guppy-backend",   type=str, default="selene_github",
                   choices=["selene_github", "selene_pypi", "helios"],
                   help="Guppy sub-backend (default selene_github)")
    p.add_argument("--guppy-shots",     type=int, default=4096)
    p.add_argument("--guppy-iters",     type=int, default=8)
    p.add_argument("--guppy-eth",       type=float, default=0.30,
                   help="Guppy energy threshold for low-E pair collection")
    # ── D-Wave backend ────────────────────────────────────────────────────────
    p.add_argument("--dwave",           action="store_true",
                   help="Add D-Wave/Neal QUBO samples to pool (Neal local, no token)")
    p.add_argument("--dwave-real",      action="store_true",
                   help="Use real D-Wave QPU via Leap (requires --dwave-token)")
    p.add_argument("--dwave-token",     type=str, default="",
                   help="D-Wave Leap API token (or set DWAVE_API_TOKEN env var)")
    p.add_argument("--dwave-endpoint",  type=str,
                   default="https://cloud.dwavesys.com/sapi/")
    p.add_argument("--dwave-reads",     type=int, default=1000)
    p.add_argument("--dwave-probes",    type=int, default=512)
    p.add_argument("--dwave-eth",       type=float, default=0.30,
                   help="D-Wave energy threshold for low-E pair collection")
    args = p.parse_args()
    ...
    print(f"  GUPPY_OK     : {'ON' if GUPPY_OK else 'OFF pip install guppylang'}  "
          f"| --guppy: {'ON' if args.guppy else 'OFF'}")
    print(f"  DWAVE_OK     : {'ON' if DWAVE_OK else 'OFF pip install dwave-ocean-sdk'}  "
          f"| NEAL_OK: {'ON' if NEAL_OK else 'OFF pip install neal'}  "
          f"| --dwave: {'ON (QPU)' if args.dwave_real else 'ON (Neal)' if args.dwave else 'OFF'}")
    ...
    kw = dict(
        ...
        # ── Guppy / SELENE ──────────────────────────────────────────────────
        use_guppy_backend   = args.guppy,
        guppy_backend_type  = args.guppy_backend,
        guppy_shots         = args.guppy_shots,
        guppy_iters         = args.guppy_iters,
        guppy_energy_thresh = args.guppy_eth,
        # ── D-Wave / Neal ────────────────────────────────────────────────────
        use_dwave_backend   = args.dwave or args.dwave_real,
        dwave_token         = args.dwave_token or os.getenv("DWAVE_API_TOKEN", ""),
        dwave_endpoint      = args.dwave_endpoint,
        use_real_dwave      = args.dwave_real,
        dwave_num_reads     = args.dwave_reads,
        dwave_n_probes      = args.dwave_probes,
        dwave_energy_thresh = args.dwave_eth,
    )
